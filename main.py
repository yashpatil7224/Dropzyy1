...
def generate_order_pdf_invoice(order_id: str, date_str: str, customer_info: dict, items: list, total_amount: float) -> bytes:
    ...
    story.append(Paragraph("For support or order tracking, visit Dropzyy app or email support@freshkart.com", styles['Normal']))
    ...
    <body>
      <div class="bill-card">
        ...
        <p style="text-align: center; color: #64748b; font-size: 13px; margin-top: 25px;">
          Thank you for ordering with Dropzyy! If you have any questions, reply to this email or contact support.
        </p>
      </div>
    </body>
    </html>
...
def update_order_status(order_id: str, payload: dict, db: Session = Depends(get_db)):
    ...
    # When order status is marked DELIVERED, send the official Dropzyy Invoice Bill HTML directly to the customer's email!
    if str(new_status).lower() == "delivered":
        try:
            ...
            if c_email and "@" in c_email:
                ...
                print(f"[DELIVERY BILL SENT] Official Dropzyy Bill sent to customer email {c_email}!")
